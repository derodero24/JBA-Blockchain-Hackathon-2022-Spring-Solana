...
const slideFetchData1:TansuNftAccount = {totalFee:1.2, 
                                          ...
                                          metaplex:{name:"dummy", imageUri:"https://ipfs.io/ipfs/QmeAP2RBjug2aVLHhk99nKJDxa7RPyBFG4mf59VmUoyZ5h"},
                                          shareholders:[{publicKey:dummyAdd.publicKey, fee:0.2}]
                                        };

export function SlideMarketPage() {
  const [slideNFTs, setSlideNFTs] = React.useState<TansuNftAccount[]>([slideFetchData1, slideFetchData1, slideFetchData1]);
      ...
      {slideNFTs.map(nft=>{
        return(
          <div key={nft.metaplex.name}>
            <Box margin={5}>
              <SlideMarketCard name={nft.metaplex.name} img_url={nft.metaplex.imageUri} totalFee={nft.totalFee} id={1} />
          </Box>
          </div>
        );
      })}
